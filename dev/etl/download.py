...
def build_data_dir(root: str, permission_override: bool, additional: Optional[Sequence[str]] = None) -> Dict[str, Path]:
    '''
...
def download(data_source: str, 
    ...
    if data_source.lower() == "gadm":
        gadm_mapping = mappings.dropna()\
            ...
            .set_index("country")\
            .to_dict()["gadm_name"]
        get_gadm_data(data_root, gadm_mapping, overwrite, verbose, permission_override)
    elif data_source.lower() == "geofabrik":
        ...
        raise Exception(f"Data souce must be one either 'gadm' or 'geofabrik' but {data_source} was provided")