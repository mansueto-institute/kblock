...
def index_building_points(gadm_block_data: gpd.GeoDataFrame, bldg_data: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
    """
    Joins GeoDataFrame of enclosed block geometries to a 
    GeoDataFrame containing building footprint centroids
    Args:
        gadm_block_data: GeoDataFrame output returned from build_blocks() function, requires CRS WGS 84 EPSG 4326 or 3395
...
def compute_k(block_id: str, block_col: str, block_data: gpd.GeoDataFrame, bldg_data: gpd.GeoDataFrame, street_linestrings: Union[pygeos.Geometry, gpd.GeoDataFrame] = None, buffer_streets: bool = False, include_geometry: bool = True) -> Union[gpd.GeoDataFrame, pd.DataFrame]:
    """
    ...
    Args:
        ...
        bldg_data: GeoDataFrame output returned from index_buildings() or index_building_points() function, requires CRS WGS 84 EPSG 4326 (or EPSG 3395 for faster performance)
...
def compute_layers(block_id: str, block_col: str, block_data: gpd.GeoDataFrame, bldg_data: gpd.GeoDataFrame, street_linestrings: Union[pygeos.Geometry, gpd.GeoDataFrame] = None, buffer_streets: bool = False) -> gpd.GeoDataFrame:
    """
...
def main(log_file: Path, country_chunk: list, chunk_size: int, core_count: int, blocks_dir: Path, streets_dir: Path, buildings_dir: Path, population_dir: Path, output_dir: Path):
    ...
    # Make directory
    kindex_dir =  str(output_dir) + '/kindex'
    Path(kindex_dir).mkdir(parents=True, exist_ok=True)
    dask_dir =  str(output_dir) + '/kindex' + '/dask'
    Path(dask_dir).mkdir(parents=True, exist_ok=True)
    logging.info(f"kindex_dir: {kindex_dir}")
    
    # Check if country is completed
    output_file_list = list(filter(re.compile("kindex_").match, sorted(list(os.listdir(Path(kindex_dir))))))
    output_country_list = [(re.sub('kindex_', '', re.sub('.parquet', '', i))) for i in output_file_list] 
    ...
    for country_code in country_list:
        ...
        # Define GADM list based on country block file
        ...
        for i in gadm_dict.items():
            ...
            try:
                street_network = gpd.read_parquet(path = Path(streets_dir) / f'streets_{country_code}.parquet', memory_map = True, filters = [('gadm_code', 'in', gadm_chunk)]).to_crs(3395)
                street_network = street_network[street_network['geometry'].notnull()].to_crs(3395)
                street_network = from_shapely_srid(geometry = street_network, srid = 3395)
            except Warning:
            ...
            pool = multiprocessing.Pool(processes= number_of_cores) #, initializer = initialize_stdout(print_file = print_stdout) 
            output = pool.map(functools.partial(compute_k, block_col = 'block_id', block_data = blocks, bldg_data = buildings, street_linestrings = street_network, buffer_streets = True, include_geometry = False), block_list)
            pool.close() 
            pool.join()
        ...
        k_bulk.to_parquet(path = Path(kindex_dir) / f'kindex_{country_code}.parquet')
