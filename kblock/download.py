...
def get_osm_lines(country_code: str) -> gpd.GeoDataFrame:
    """
    ...
    pyrosm.data.get_data(dataset = geofabrik_name, update = True, directory = temp_dir.name)  
    print(os.listdir(temp_dir.name))
    extract = pyrosm.OSM(str(temp_dir.name + f'/{geofabrik_name}-latest.osm.pbf'))
...
def get_gadm_data(country_code: str) -> gpd.GeoDataFrame:
    """
    ...
    gadm_file = gadm_dir_to_path(gadm_dir = temp_dir.name)
