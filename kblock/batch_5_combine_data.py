...
def main(log_file: Path, country_chunk: list, blocks_dir: Path, population_dir: Path, buildings_dir: Path, complexity_dir: Path, streets_dir: Path, crosswalks_dir: Path, output_dir: Path):
    ...
    for country_code in country_list:
        ...
        for boundary in boundary_list:
            print(boundary)
            boundary_polys = subset_boundaries[subset_boundaries['urban_layer_code'] == boundary][['geometry']]
            ...
            for tag in streets['osm_highway_tag'].unique():
                ...
                if not pygeos.is_empty(streets_lines).all():
                    ...
                    streets_lines = streets_lines[['urban_layer_code','osm_highway_tag','street_length_meters','mean_linearity']]
                    streets_lines[['street_length_meters','mean_linearity']] = streets_lines[['street_length_meters','mean_linearity']].fillna(0)
                    streets_lines = streets_lines.groupby(['urban_layer_code','osm_highway_tag']).sum(['street_length_meters','mean_linearity']).reset_index()
        ...
        del streets, subset_boundaries, boundary_list, boundary_residual, boundary_polys, streets_lines
    ...
    streets_metrics = streets_metrics[['urban_layer_code', 'osm_highway_tag', 'highway_classification', 'vehicular_classification', 'street_length_meters','mean_linearity']]

    logging.info(f'Writing files.')
    Path(str(output_dir) + '/region_data').mkdir(parents=True, exist_ok=True)
    streets_metrics.to_parquet(Path(output_dir) / 'region_data' / 'streets_metrics.parquet')
    del streets_metrics
    ...
    all_complexity = pd.DataFrame({
        ...
        'parcel_layers': pd.Series(dtype='object'),
    ...
    Path(str(output_dir) + '/africa_data').mkdir(parents=True, exist_ok=True)
    all_data.to_parquet(path = Path(output_dir) / 'africa_data' / f'africa_geodata.parquet')
    all_data[['block_id', 'block_geohash', 'country_code', 'country_name', 'area_type', 'urban_center_name', 'agglosname', 'block_hectares', 'building_count', 'average_building_area_m2', 'k_complexity', 'landscan_population_un', 'landscan_population_un_log', 'landscan_population_un_density_hectare', 'landscan_population_un_density_hectare_log', 'landscan_population_un_per_building_area_m2', 'geometry']].to_parquet(path = Path(output_dir) / 'africa_data' / f'africa_map.parquet')
    all_data.drop(columns='geometry').to_parquet(path = Path(output_dir) / 'africa_data' / f'africa_data.parquet')
    all_data.drop(columns='geometry').to_csv(path_or_buf = Path(output_dir) / 'africa_data' / f'africa_data.csv', index=False)
    # Path(str(output_dir) + '/africa_data' + '/gpkg').mkdir(parents=True, exist_ok=True)
    # all_data[['block_id','geometry']].to_file(filename = Path(output_dir) / 'africa_data' / 'gpkg' / f'africa_geodata.gpkg', driver='GPKG')
    ...
    Path(str(output_dir) + '/region_data').mkdir(parents=True, exist_ok=True)
    all_regions.to_parquet(path = Path(output_dir) / 'region_data' / 'aggregate_regional_geodata.parquet')
    all_regions.drop(columns='geometry').to_parquet(path = Path(output_dir) / 'region_data' /  'aggregate_regional_data.parquet')
    all_regions.to_file(filename = Path(output_dir) / 'region_data' / 'aggregate_regional_geodata.gpkg', driver='GPKG')
    all_regions.drop(columns='geometry').to_csv(path_or_buf = Path(output_dir) / 'region_data' / 'aggregate_regional_data.csv', index=False)
    ...
    for country_code in country_list:
        logging.info(f'{country_code}')
        all_data_sub = all_data[all_data['country_code'] == country_code]
        Path(str(output_dir) + '/country_data' + f'/{country_code}').mkdir(parents=True, exist_ok=True)
        all_data_sub.to_parquet(Path(output_dir) / 'country_data' / f'{country_code}' / f'{country_code}_geodata.parquet')
        all_data_sub.to_file(Path(output_dir) / 'country_data' / f'{country_code}' / f'{country_code}_geodata.gpkg', driver="GPKG")
        all_data.drop(columns='geometry').to_csv(path_or_buf = Path(output_dir) / 'country_data' / f'{country_code}' / f'{country_code}_data.csv', index=False)
